...
def LargeCitypathloss(distance,f,ht,hr):
    ahrl= 3.2*(2* math.log(11.75*hr,10))-4.97
    L=69.55 +26.16*math.log(f,10) - 13.82*math.log(ht,10) -ahrl +(44.9 -6.55*math.log(ht,10))*math.log(distance,10)
    return L

def LargeCitydistanceFrom_loss(L, f,ht,hr):
...
m = folium.Map(location=sensor_location, zoom_start=14)
zoom_level = 14
Freq = 903.9
txs=10
txj=22
...
radius=LargeCitydistanceFrom_loss(sensitivity+txs, Freq,8,8)
print(radius)
...
for key in hotspots:
    ...
    L_sh = LargeCitypathloss(d, Freq,8,8)

    L_jh= - txs + txj + L_sh + 3   # Tx(s)=15 Tx(j)=21
    d_jh=LargeCitydistanceFrom_loss(L_jh, Freq,2,8)
