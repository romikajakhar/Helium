...
m = folium.Map(location=sensor_location, zoom_start=16)
zoom_level = 16
Freq = 903.9
txs=15
txj=21
...
radius=distanceFrom_loss(125-txs, Freq)
...
for key in hotspots:
    ...
    L_sh = pathloss(d, Freq)

    L_jh= - txs + txj + L_sh + 3   # Tx(s)=15 Tx(j)=21
    d_jh=distanceFrom_loss(L_jh, Freq)
